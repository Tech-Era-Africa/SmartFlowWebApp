    ...
    <div class="w-screen h-screen bg-pageBg relative">
        ...
        <template v-if="billStore.isFetchingBill">
            ...
            <div class="w-full h-full flex flex-col gap-12 justify-center items-center">
                <Logo></Logo>
                <p class="text-gray-500  flex flex-col items-center gap-5">Fetching the bill <span class="loading loading-infinity text-blue-500"></span></p>
            </div>
            
        </template>

        <template v-if="billStore.success_FetchingBill">
            <div class="h-40 bg-invoiceBg-1 bg-cover"></div>
            <div class="lg:w-1/3 mx-auto p-8 flex flex-col gap-5 bg-white absolute left-0 right-0 top-11 rounded-lg">
                <Bill :option="{ device: billOption }"></Bill>
            </div>
        </template>
        <template v-if="billStore.failed_FetchingBill">
            ...
            <div class="w-1/3 rounded-lg flex flex-col gap-5 mx-auto bg-white p-5">
                ...
                <h1 class="font-bold text-xl text-center">Invalid Bill <Icon class="text-red-500" name="charm:circle-cross">
                    </Icon>
                </h1>
...
const billOption = ref(deviceStore.selectedDevice)
